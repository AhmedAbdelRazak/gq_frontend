...
import { getColors, readUser } from "../../apiCore";
...
const UserDashboard = () => {
	...
	const lastPurchase =
		...
		userDetails.history[userDetails.history.length - 1];
	...
	return (
		<UserDashboardWrapper>
			...
			<div className='container'>
				...
				<div className='ml-3' style={{ fontSize: "1rem", fontWeight: "bold" }}>
					Shipping details:
				</div>
				<div
					className='ml-5 row'
				...
				<div className='ml-5 row'>
						...
						lastPurchase.chosenProductQtyWithVariables.length > 0 ? (
							<>
								<div className='row mt-3'>
									{lastPurchase.chosenProductQtyWithVariables.map((p, i) => {
										return (
											<React.Fragment key={i}>
												{p.map((pp, ii) => {
													return (
														<div className='col-md-9 text-capitalize' key={ii}>
															<div className='row'>
					...
					<div className='col-md-4 mt-3'>
						...
						<span style={{ fontWeight: "bold", color: "#737373" }}>
							{Number(
								Number(lastPurchase && lastPurchase.totalAmountAfterDiscount) -
						...
						<span style={{ fontWeight: "bold", color: "#737373" }}>1.00%</span>
						<br />
						<span style={{ fontWeight: "bold" }}>COD surcharge rate:</span>{" "}
						<span style={{ fontWeight: "bold", color: "#737373" }}>
							{Number(
								(Number(lastPurchase && lastPurchase.totalAmountAfterDiscount) -
									Number(lastPurchase && lastPurchase.shippingFees)) *
				...
				<div className='ml-3' style={{ fontSize: "1rem", fontWeight: "bold" }}>
					...
					userDetails.history.length === 1
						? `${userDetails.history.length} order with ACE shop`
						: userDetails &&
						  userDetails.history &&
						  userDetails.history.length &&
						  userDetails.history.length > 1
						? `${userDetails.history.length} orders with ACE shop`
						: "Orders"}
					:
				</div>
			</div>
			<UserHistory currentPosts={userDetails.history} />
		</UserDashboardWrapper>
	);
};
